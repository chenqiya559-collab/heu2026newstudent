...
const ragSynonymGroups = [
  ...
  ['PPT','模板','答辩','汇报','演示'],
  ['助学贷款','国家助学贷款','生源地贷款','生源地信用助学贷款','校园地贷款','校园地国家助学贷款','贷款回执','回执码','受理证明','绿色通道','不交学费','暂缓缴费','贷款扣学费','学费抵扣'],
...
const ragRelations = [
  ...
  { triggers:['助学贷款','生源地贷款','校园地贷款','贷款回执','受理证明'], expands:['国家助学贷款','学费','住宿费','绿色通道','回执','辅导员','资助部门','不要重复缴费','当年通知'] },
...
function retrieve(query) {
  ...
  const scored=ragChunks.map(chunk=>{ let score=0; if(normalized&&chunk.titleHay.includes(normalized)) score+=30; if(normalized&&chunk.textHay.includes(normalized)) score+=20;
    ...
    if(['中国银行','中行','银行卡','储蓄卡','办银行卡','助学金','奖学金','奖助学金'].some(term=>normalized.includes(term))&&chunk.item.id==='campus-banking-guide') score+=45;
    if(['助学贷款','国家助学贷款','生源地贷款','校园地贷款','贷款回执','回执码','受理证明','不交学费','暂缓缴费','贷款扣学费','学费抵扣'].some(term=>normalized.includes(term))&&chunk.item.id==='student-loan-guide') score+=70;
...
function hasSpecificQuestionAnchor(question) {
  const normalized=ragNormalize(question); const anchors=['报到','材料','路线','机场','火车站','选课','培养方案','校园卡','饭卡','一卡通','餐卡','澡卡','校园码','充值','余额','挂失','补卡','电费','宿舍电费','公寓电费','电费账号','房间账号','gy0','欠费','断电','电费分摊','中国银行','中行','银行卡','储蓄卡','助学贷款','国家助学贷款','生源地贷款','校园地贷款','贷款回执','受理证明','绿色通道','不交学费','暂缓缴费','贷款扣学费','奖学金','助学金','奖助学金','国家奖学金','国奖','国家励志','国励','国防科技奖学金','困难生','家庭经济困难','困难认定','贫困认定','智慧学工','校园网','无线','wifi','网费','网络费','宽带费','上网费','充网费','缴网费','网费账号','宿舍','寝室','独卫','四人寝','四人间','上床下桌','阳台','查寝','洗澡','洗浴','浴池','澡堂','洗衣机','洗衣','吹风机','吹头发','快递','包裹','取件','驿站','社团','诈骗','哈军工','校训','学科','赞噢','集市','吃饭','食堂','餐厅','大美','小美','至美','快乐食间','小公交','校车','校园巴士','巴士','摆渡车','接驳车','购物','超市','新生采购','入学采购','集体售卖','统一购买','被子','被褥','床品','洗衣液','电话卡','手机卡','sim卡','网购','启航','北体育场','北体','剪头','理发','修手机','水果','外卖','外送','取餐','打印','复印','21b','教材','课本','二手书','图书馆','借书','借阅'];
